  ...
  <div>
    ...
    <div class="row justify-center">
      <div class="col-12 col-md-7 bgAree">
        <q-input
          v-model="CercaUtentiDaAutorizzare"
          debounce="1"
          outlined
          :dense="true"
          placeholder="Cerca Utente da Autorizzare - Inserire 3 caratteri per avviare la ricerca"
          @keypress="elencoCercaUsersFunction()"
        >
          <div
            class="autocomplete-items"
            v-if="CercaUtentiDaAutorizzare.length > 2"
          >
            <div
              class="row"
              v-for="item in elencoCercaUtentiDaAutorizzare"
              :key="item.message"
            >
              <div class="col">
                <a href="#" @click="autorizzaUtente(item)">
                  {{ item.name }} - {{ item.email }} - {{ item.role }}</a
                >
              </div>
            </div>
          </div>
...
<div v-if="value.elencoUtentiDaAutorizzare.length > 0">
    <div class="row justify-center" style="justify-center">
      <div class="col-4 col-md-2 bgAree" style="border-bottom: 1px solid black">
        <b>Nome</b>
      </div>
      <div class="col-4 col-md-2 bgAree" style="border-bottom: 1px solid black">
        <b>Email</b>
      ...
      <div class="col-1 col-md-1 bgAree" style="border-bottom: 1px solid black">
        <b></b>
      </div>
    </div>
    ...
    <div
      ...
      style="color: grey"
      v-if="value.elencoUtentiDaAutorizzare.length > 0"
    >
      <div class="col-12 col-md-7 bgAree">
        <q-btn
          color="primary"
    ...
    <div class="row justify-center">
      ...
      <div class="col-1 col-md-1 bgAree"></div>
    </div>
    <div
      class="row justify-center"
      v-for="item in value.elencoUtentiAutorizzati"
      :key="item.message"
    >
      ...
      <div class="col-1 col-md-1 bgAree">
        <q-btn
          ...
          round
          icon="delete"
          @click="cancellaUtente(item.idRiga)"
        />
      </div>
    </div>
  </div>
  ...
  methods: {
    elencoCercaUsersFunction() {
      if (this.CercaUtentiDaAutorizzare.length > 1) {
        Axios.get(
          this.linkApi +
            "/getCercaUsersDaAutorizzare/" +
            this.value.idprogetto +
            "/" +
    ...
    autorizzaUtenti() {
      ...
      Axios.post(this.linkApi + "/SetAutorizzaUtenti", sendDaAutorizzare).then(
        (Response) => {
          if (Response.data.response == "ok") {
            this.elencoUtentiAutorizzati = [];
            Axios.get(
              this.linkApi + "/getUtentiAutorizzati/" + this.value.idprogetto
            ).then((ResponseAutorizzati) => {
              ResponseAutorizzati.data["listaUsersAssociatiAlProgetto"].forEach(
                (element) => {
    ...
    cancellaUtente(id) {
      Axios.delete(this.linkApi + "/cancellaAutorizzazione/" + id).then(
        (response) => {
          if (response.data.response == "ok") {
